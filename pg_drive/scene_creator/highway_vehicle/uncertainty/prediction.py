...
from city_drive.highway_env.road.road import Route, LaneIndex, Road
from city_drive.highway_env.types import Vector
from city_drive.highway_env.vehicle import Vehicle
from city_drive.highway_env.vehicle.behavior import LinearVehicle
from city_drive.highway_env.vehicle.controller import MDPVehicle

if TYPE_CHECKING:
    from city_drive.highway_env.road import RoadObject
...
class IntervalVehicle(LinearVehicle):
    ...
    def observer_step(self, dt: float) -> None:
        ...
        for lane_index in lanes:
            ...
            phi_b_i_lane = np.transpose(np.array([[0, 0], intervals_product(lateral_i, i_v_i)]))
    ...
    def predictor_init(self) -> None:
        ...
        if not self.longitudinal_lpv:
            ...
            center = [
                -self.DISTANCE_WANTED - self.target_speed * self.TIME_WANTED, 0, self.target_speed, self.target_speed
            ]
            noise = 1
            b = np.eye(4)
            d = np.array([[1], [0], [0], [0]])
    ...
    def check_collision(self, other: Union['Vehicle', 'RoadObject']) -> None:
        ...
        if not utils.point_in_rectangle(other.position, self.interval.position[0] - self.LENGTH,
