...
def test_top_down_rendering():
    for env in [
            ...
            WaymoEnv(dict(num_scenario=1, start_scenario_index=0)),
            WaymoEnv(dict(num_scenario=1, start_scenario_index=1)),
            WaymoEnv(dict(num_scenario=1, start_scenario_index=2)),
    ]:
        try:
            for _ in range(5):
                o = env.reset()
