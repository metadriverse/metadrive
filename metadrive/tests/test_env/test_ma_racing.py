...
def test_ma_racing_reset():
    env = MultiAgentRacingEnv({"horizon": 50, "num_agents": 4})
    try:
        _check_spaces_before_reset(env)
        obs, _ = env.reset()
        _check_spaces_after_reset(env, obs)
    ...
    env = MultiAgentRacingEnv({"horizon": 100, "num_agents": 2, "success_reward": 777})
    try:
        _check_spaces_before_reset(env)
        success_count = 0
        agent_count = 0
...
def test_ma_racing_close_spawn():
    ...
    MultiAgentRacingEnv._DEBUG_RANDOM_SEED = 1
    env = MultiAgentRacingEnv(
        {
            # "use_render": True,
            "horizon": 50,
    ...
    try:
        ...
        env.close()
        MultiAgentRacingEnv._DEBUG_RANDOM_SEED = None


def test_ma_racing_reward_done_alignment_1():
    # out of road
    env = MultiAgentRacingEnv({"horizon": 200, "num_agents": 4, "out_of_road_penalty": 777, "crash_done": False})
    try:
        _check_spaces_before_reset(env)
        obs, _ = env.reset()
        _check_spaces_after_reset(env, obs)
...
def test_ma_racing_reward_done_alignment_2():
    # crash 2
    env = MultiAgentRacingEnv(
        {
            "map_config": {
...
def test_ma_racing_reward_sign():
    """
            ...
            for num_agents in [16, 32]:
                for num_others in [0, 2, 4, 8]:
                    for crash_vehicle_penalty in [0, 5]:
                        ...
                        env = MultiAgentRacingEnv(env_config)
...
def test_ma_racing_no_short_episode():
    env = MultiAgentRacingEnv({
        "horizon": 300,
        "num_agents": 2,
    })
...
def test_ma_racing_horizon_termination(vis=False):
    # test horizon
    env = MultiAgentRacingEnv({
        "horizon": 100,
        "num_agents": 2,
...
def test_ma_racing_40_agent_reset_after_respawn():
    ...
    env = MultiAgentRacingEnv({"horizon": 50, "num_agents": 2})
    try:
        _check_spaces_before_reset(env)
        obs, _ = env.reset()
        _check_spaces_after_reset(env, obs)
...
def test_ma_no_reset_error():
    ...
    env = MultiAgentRacingEnv({"horizon": 300, "num_agents": 2, "delay_done": 0, "use_render": False})
    try:
        _check_spaces_before_reset(env)
        obs, _ = env.reset()
        _check_spaces_after_reset(env, obs)
...
def test_randomize_spawn_place():
    last_pos = {}
    env = MultiAgentRacingEnv({"num_agents": 4, "use_render": False, "force_seed_spawn_manager": False})
    try:
        obs, _ = env.reset()
        for step in range(100):
            act = {k: [1, 1] for k in env.vehicles.keys()}
