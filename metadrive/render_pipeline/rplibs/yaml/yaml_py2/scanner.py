...
class Scanner(object):
    ...
    def fetch_more_tokens(self):
        ...
        raise ScannerError(
            ...
            "found character %r that cannot start any token" % ch.encode('utf-8'), self.get_mark()
        )
    ...
    def scan_directive_name(self, start_mark):
        ...
        if not length:
            raise ScannerError(
                ...
                "expected alphabetic or numeric character, but found %r" % ch.encode('utf-8'), self.get_mark()
            )
    ...
    def scan_yaml_directive_value(self, start_mark):
        ...
        if self.peek() != '.':
            raise ScannerError(
                "while scanning a directive", start_mark,
                "expected a digit or '.', but found %r" % self.peek().encode('utf-8'), self.get_mark()
            )
        self.forward()
    ...
    def scan_yaml_directive_number(self, start_mark):
        ...
        if not (u'0' <= ch <= u'9'):
            raise ScannerError(
                "while scanning a directive", start_mark, "expected a digit, but found %r" % ch.encode('utf-8'),
                self.get_mark()
            )
        length = 0
    ...
    def scan_tag_directive_handle(self, start_mark):
        ...
        if ch != u' ':
            raise ScannerError(
                "while scanning a directive", start_mark, "expected ' ', but found %r" % ch.encode('utf-8'),
                self.get_mark()
            )
        return value
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        if ch not in u'\0\r\n\x85\u2028\u2029':
            raise ScannerError(
                "while scanning a directive", start_mark,
                "expected a comment or a line break, but found %r" % ch.encode('utf-8'), self.get_mark()
            )
    ...
    def scan_tag(self):
        ...
        if ch == u'<':
            ...
            if self.peek() != u'>':
                raise ScannerError(
                    "while parsing a tag", start_mark, "expected '>', but found %r" % self.peek().encode('utf-8'),
                    self.get_mark()
                )
            self.forward()
        elif ch in u'\0 \t\r\n\x85\u2028\u2029':
        ...
        if ch not in u'\0 \r\n\x85\u2028\u2029':
            raise ScannerError(
                "while scanning a tag", start_mark, "expected ' ', but found %r" % ch.encode('utf-8'), self.get_mark()
            )
        value = (handle, suffix)
        end_mark = self.get_mark()
    ...
    def scan_block_scalar_indicators(self, start_mark):
        ...
        if ch not in u'\0 \r\n\x85\u2028\u2029':
            raise ScannerError(
                "while scanning a block scalar", start_mark,
                "expected chomping or indentation indicators, but found %r" % ch.encode('utf-8'), self.get_mark()
            )
    ...
    def scan_plain(self):
        ...
        while True:
            ...
            if (self.flow_level and ch == u':' and self.peek(length + 1) not in u'\0 \t\r\n\x85\u2028\u2029,[]{}'):
                self.forward(length)
                raise ScannerError(
    ...
    def scan_tag_handle(self, name, start_mark):
        ...
        if ch != u'!':
            raise ScannerError(
                "while scanning a %s" % name, start_mark, "expected '!', but found %r" % ch.encode('utf-8'),
                self.get_mark()
            )
        length = 1
        ch = self.peek(length)
    ...
    def scan_tag_uri(self, name, start_mark):
        ...
        if not chunks:
            raise ScannerError(
                "while parsing a %s" % name, start_mark, "expected URI, but found %r" % ch.encode('utf-8'),
                self.get_mark()
            )
