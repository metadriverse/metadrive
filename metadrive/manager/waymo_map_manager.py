...
class WaymoMapManager(BaseManager):
    ...
    def update_route(self, data):
        init_state = WaymoTrafficManager.parse_vehicle_state(data["tracks"][data["sdc_index"]]["state"], self.engine.global_config["case_start_index"])
        last_state = WaymoTrafficManager.parse_vehicle_state(data["tracks"][data["sdc_index"]]["state"], self.engine.global_config["case_end_index"])
